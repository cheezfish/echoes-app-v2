// admin-portal/admin.js - SCOPE FIX
...
let adminLoginForm, adminLoginError, adminLoginSection, adminDashboardSection, adminLogoutBtn,
    echoesTableBody, usersTableBody, adminMapContainer;

// --- APP STATE (Declared globally) ---
let adminMap, adminMarkers;
let adminToken = null;

// Wait for the DOM to be fully loaded before assigning elements and running the app
document.addEventListener('DOMContentLoaded', () => {
    // Assign elements now that they exist
...
function initializeAdminMap() {
    // Now this function can correctly access the global adminMapContainer
...
async function fetchAllEchoesForAdmin() {
    echoesTableBody.innerHTML = '<tr><td colspan="9">Loading...</td></tr>';
    const echoes = await fetchData('/admin/api/echoes');
    if (echoes) {
        renderEchoesTable(echoes);
        renderEchoesOnAdminMap(echoes);
    } else {
        echoesTableBody.innerHTML = '<tr><td colspan="9" class="error">Failed to fetch echoes.</td></tr>';
    }
}
...
async function fetchAllUsersForAdmin() {
    ...
    if (users) {
        ...
        usersTableBody.innerHTML = '<tr><td colspan="5" class="error">Failed to fetch users.</td></tr>';
    }
}

function renderEchoesTable(echoes) {
    echoesTableBody.innerHTML = "";
    if (echoes.length === 0) return void(echoesTableBody.innerHTML = '<tr><td colspan="9">No echoes found.</td></tr>');
    echoes.forEach(echo => {
...
async function handleDeleteEcho(e) {
    ...
    try {
        const response = await fetch(`${API_URL}/admin/api/echoes/${echoId}`, { method: "DELETE", headers: { Authorization: `Bearer ${adminToken}` } });
        const data = await response.json();
...
async function handleToggleAdmin(e) {
    ...
    if (!confirm(`${isAdmin ? "Remove admin from" : "Make admin"} user ID ${userId}?`)) return;
    try {
        const response = await fetch(`${API_URL}/admin/api/users/${userId}/toggle-admin`, { method: "PUT", headers: { Authorization: `Bearer ${adminToken}` } });
        const data = await response.json();
...
function renderUsersTable(users) {
    ...
    if (users.length === 0) return void(usersTableBody.innerHTML = '<tr><td colspan="5">No users found.</td></tr>');
    users.forEach(user => {
