...
document.addEventListener('DOMContentLoaded', () => {
    ...
    infoPanelTitleEl = document.getElementById("info-panel-title"); // Corrected from w3wAddressEl for this purpose
    ...
    closeModalBtn = document.querySelector("#auth-modal .close-btn"); // Scoped selector
    ...
    closeClusterModalBtn = document.querySelector("#cluster-modal .close-btn"); // Scoped selector
...
function checkLoginState(){const e=localStorage.getItem("echoes_token");e?(userToken=e,loggedInUser=JSON.parse(atob(e.split(".")[1])).user.username,updateUIAfterLogin()):updateUIAfterLogout();updateInfoPanelTextAndButtonState()}
...
function renderEchoesOnMap(echoes){if(!markers||!echoMarkersMap)return;echoes.forEach(echo=>{if(echo.lat&&echo.lng){const ageInHours=(new Date-new Date(echo.last_played_at))/36e5,icon=ageInHours<48?echoIconFresh:ageInHours<360?echoIconStable:echoIconFading,marker=L.marker([echo.lat,echo.lng],{icon});marker.echoData=echo,marker.bindPopup(createEchoPopup(echo)),echoMarkersMap.set(echo.id,marker),markers.addLayer(marker)}})}
...
function handleFindMeClick(){updateStatus("Locating your position...","info"),isManualFind=!0,navigator.geolocation.getCurrentPosition(onLocationSuccess,onLocationError,{enableHighAccuracy:!0,timeout:1e4,maximumAge:0})}
